...
def test_validate_config_accepts_supervisor_provider_without_remote_llm_fields(
    tmp_path: Path,
):
    data = _valid_config_data()
    data["llm"] = {
...
def test_rcconfig_from_dict_parses_supervisor_settings(tmp_path: Path):
    data = _valid_config_data()
    data["llm"] = {
        "provider": "supervisor",
        "primary_model": "codex-supervisor",
    ...
    assert config.llm.provider == "supervisor"
    assert config.llm.primary_model == "codex-supervisor"
    assert config.llm.fallback_models == ("codex-fallback",)
    assert config.llm.supervisor.exchange_dir == str(tmp_path / "exchange")
    assert config.llm.supervisor.timeout_sec == 123
    assert config.llm.supervisor.poll_interval_sec == 0.5
