...
def test_print_doctor_report_fail(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    health.print_doctor_report(report)
    out = capsys.readouterr().out
    assert "❌" in out or "ERR" in out
    assert "⚠️" in out or "WARN" in out
...
def _write_supervisor_config(path: Path) -> None:
    _ = path.write_text(
        """\
project:
  name: demo
...
def test_check_supervisor_exchange_pass(tmp_path: Path) -> None:
    exchange_dir = tmp_path / "exchange"

    result = health.check_supervisor_exchange(str(exchange_dir))

    assert result.status == "pass"
    assert exchange_dir.exists()
    assert "Supervisor exchange ready" in result.detail


def test_run_doctor_supervisor_skips_http_checks(tmp_path: Path) -> None:
    config_path = tmp_path / "config.yaml"
    _write_supervisor_config(config_path)
    with (
        patch.object(
            health, "check_python_version",
            return_value=health.CheckResult("python_version", "pass", "ok"),
        ...
        patch.object(
            health, "check_supervisor_exchange",
            return_value=health.CheckResult("supervisor_exchange", "pass", "ok"),
        ),
        patch.object(
            health, "check_sandbox_python",
            return_value=health.CheckResult("sandbox_python", "pass", "ok"),
    ...
    assert "api_key_valid" not in check_names
    assert "model_chain" not in check_names
    assert "supervisor_exchange" in check_names
    assert report.overall == "pass"
    assert len(report.checks) == 7
