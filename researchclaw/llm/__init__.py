...
def create_llm_client(config: RCConfig) -> LLMClient | ACPClient | SupervisorClient:
    """Factory: return the right LLM client based on ``config.llm.provider``.

    - ``"acp"`` → :class:`ACPClient` (spawns an ACP-compatible agent)
    - ``"supervisor"`` → :class:`SupervisorClient` (filesystem exchange)
    ...
    if config.llm.provider == "supervisor":
        from researchclaw.llm.supervisor_client import SupervisorClient as _Supervisor

        return _Supervisor.from_rc_config(config)
