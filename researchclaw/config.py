...
class SupervisorConfig:
    """External supervisor exchange settings."""
...
def validate_config(
    ...
    *,
    ...
    for key in REQUIRED_FIELDS:
        # ACP/supervisor providers don't need remote endpoint credentials
        if llm_provider in {"acp", "supervisor"} and key in (
            "llm.base_url",
            "llm.api_key_env",
        ):
            continue
...
def _parse_llm_config(data: dict[str, Any]) -> LlmConfig:
    ...
    supervisor_data = data.get("supervisor") or {}
    return LlmConfig(
        ...
        supervisor=SupervisorConfig(
            exchange_dir=supervisor_data.get(
                "exchange_dir", ".researchclaw-supervisor"
            ),
            timeout_sec=int(supervisor_data.get("timeout_sec", 3600)),
            poll_interval_sec=float(
                supervisor_data.get("poll_interval_sec", 1.0)
            ),
        ),
    )
