...
def check_supervisor_exchange(exchange_dir: str) -> CheckResult:
    path = Path(exchange_dir or ".researchclaw-supervisor")
    try:
        ...
        return CheckResult(
            name="supervisor_exchange",
            status="fail",
            detail=f"Supervisor exchange directory is not writable: {path} ({exc})",
            fix="Set llm.supervisor.exchange_dir to a writable path",
        )
    return CheckResult(
        name="supervisor_exchange",
        status="pass",
        detail=f"Supervisor exchange ready: {path}",
    )
...
def run_doctor(config_path: str | Path) -> DoctorReport:
    ...
    supervisor_exchange_dir = ".researchclaw-supervisor"
    ...
    try:
        ...
        supervisor_exchange_dir = config.llm.supervisor.exchange_dir
    ...
    if provider == "acp":
        ...
        checks.append(check_supervisor_exchange(supervisor_exchange_dir))
    else:
...
def print_doctor_report(report: DoctorReport) -> None:
    ...
    fancy_icons = {"pass": "✅", "fail": "❌", "warn": "⚠️"}
    ascii_icons = {"pass": "OK", "fail": "ERR", "warn": "WARN"}

    encoding = (getattr(sys.stdout, "encoding", None) or "utf-8").lower()
    try:
        "".join(fancy_icons.values()).encode(encoding)
        icon_by_status = fancy_icons
    except (LookupError, UnicodeEncodeError):
        icon_by_status = ascii_icons
